 ...
 * }
 */
router.post('/deduplicate', requireAdmin, async (req, res, next) => {
  try {
 ...
 * Hinweis: Mahlzeiten haben keine Bildpfade in der DB (nur thumbnail_base64 als Base64-String)
 */
async function deduplicateMeals(db, dryRun) {
  ...
  for (const group of duplicates) {
    ...
    // Lade alle Mahlzeiten dieser Gruppe (nur IDs, keine Bildpfade)
    // Hinweis: Mahlzeiten haben keine Bildpfade in der DB (nur thumbnail_base64 als Base64-String)
    ...
    if (!dryRun && idsToDelete.length > 0) {
      // Erstelle Platzhalter basierend auf idsToDelete (nicht ids!)
      const deletePlaceholders = idsToDelete.map(() => '?').join(',');
      
      // Lösche zuerst die Komponenten
      const deleteComponentsStmt = db.prepare(`DELETE FROM meal_components WHERE meal_id IN (${deletePlaceholders})`);
      deleteComponentsStmt.run(...idsToDelete);
      
      // Lösche die Mahlzeiten selbst (inkl. thumbnail_base64 - wird automatisch mit gelöscht)
      const deleteMealsStmt = db.prepare(`DELETE FROM meals WHERE id IN (${deletePlaceholders})`);
  ...
  return {
    ...
    totalDuplicates: duplicates.reduce((sum, g) => sum + (g.count - 1), 0),
    deletedCount: dryRun ? 0 : deletedCount,
    deletedImages: 0, // Keine Bilddateien für Mahlzeiten (nur Base64-Strings in DB)
    groups: duplicateGroups
  };
}
        ...
        for (const imagePath of imagesToDelete) {
          try {
            ...
            if (path.isAbsolute(imagePath)) {
              ...
              // Versuche verschiedene Basispfade
              const possiblePaths = [
                path.join(__dirname, '../data/photos', path.basename(imagePath)),
                path.join(__dirname, '..', imagePath),
                path.join(process.cwd(), imagePath),
                imagePath // Falls bereits vollständig relativ zum Server-Root
              ];
              
              fullPath = possiblePaths.find(p => fs.existsSync(p));
              if (!fullPath) {
                // Fallback: Verwende den ersten möglichen Pfad
            ...
            if (fs.existsSync(fullPath)) {
              ...
              logger.warn(`Bild nicht gefunden: ${fullPath} (Original: ${imagePath})`);
            }
          } catch (error) {
